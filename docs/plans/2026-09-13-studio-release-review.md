...
**2026-09-25: production migrations 202609130001-3 applied with Doug's approval; field test (two real phones, revocation against a real LiveKit room) still pending.**
...
Manual Pickle, Turkey, Tiny violin and Heating up cues, plus explicit eight-second custom sound recording/preview/share. Built-ins are original synthesized audio generated by tools/generate-soundboard.ts, not recordings of the commercial toy. Web and native use identical PCM assets. Fresh attributed celebrations appear silently by default. Local personal audio and the single Lane speaker role are explicit opt-ins; lease expiry and host mute stop playback. No automatic score trigger or live voice chat is enabled.

Additional review: live-soundboard.ts, soundboard route/integration and migration003. Private audio is fetched only after current session access checks. Strict bounded PCM validation, uploader/host removal, 10 clip limit and 2,000 cue limit. A session-row-lock RPC serializes mute, five-second cooldown and 30-second speaker claims. Independently tested in disposable PostgreSQL: concurrent speaker claims produce one owner; simultaneous plays produce one accepted event; 12 parallel clip saves produce 10 successes and two capacity errors; anonymous/client RPC and table access denied; cascade cleanup passes. Production untouched.

Soundboard validation: 22 browser checks with real WebAudio playback and synthetic microphone capture, delayed-download/remote-mute regression, mobile widths, and reduced motion. Browser-recorded WAV and native canonical WAV both accepted by the production parser locally. All four bundled cues pass format/peak checks and are byte-identical across clients. This is local/synthetic evidence, not physical speaker or microphone verification. No product sounds or recordings were broadcast to real sessions during testing.

Final soundboard checks: 101 web tests / 61,373 assertions, production build, 22 browser interaction checks, 15 native model checks including a delayed 26-second speaker claim, native phone/iPad renders, 36 contrast pairs and 16 prominent-button guards. Signed build14 contains all four matching WAV assets and the late-speaker-confirmation guard; codesign verification passes. Physical iPhone microphone and alley speaker playback still need device validation.
