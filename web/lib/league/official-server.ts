...
export async function loadOfficialWeek(season: string, week: number): Promise<OfficialWeek | null> {
  ...
  const [teamWeeks, bowlerWeeks, bowlers]: Row[][] = await Promise.all([
  ...
  return { pointsWon: Number(ours.week_points_won), ourHdcpGames: ours.hdcp_games ?? [], theirHdcpGames: theirs?.hdcp_games ?? [], ours: roster(ourTeam), theirs: roster(ours.opponent_team_id) };
}
