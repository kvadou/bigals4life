...
export async function GET(_:Request,context:Context){
  ...
  try{
    ...
    return rows[0]?Response.json(rows[0],{headers:{"Cache-Control":"no-store"}}):Response.json({error:"Team scorebook not found."},{status:404});
  }catch{return Response.json({error:"Could not load shared scores. Please retry."},{status:503})}
}
export async function PUT(request:Request,context:Context){
  if(!sameOrigin(request))return Response.json({error:"Use the scorebook website to save."},{status:403});
  const id=uuidSchema.safeParse((await context.params).id);
  if(!id.success)return Response.json({error:"Invalid team link."},{status:400});
  let update;
