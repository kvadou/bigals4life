...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (identity) await database("scorebook_members", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ scorebook_id: rows[0].id, user_id: identity.user.id, role: "owner", added_by: identity.user.id }) });
