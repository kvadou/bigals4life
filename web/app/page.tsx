...
import { GameRows, MatchHero, fmt } from "./components/match-hero";

type Standings = { teams: { name: string; place: number; pointsWon: number; pointsLost: number; ours: boolean }[]; roster: { name: string; average: number; games: number[] | null; total: number | null }[] };
...
export default function Home() {
  ...
  const highSeries = week ? week.series.map((s, i) => ({ s: s ?? 0, i })).sort((a, b) => b.s - a.s)[0] : null;
  ...
  return <main>
    ...
    <div className="entry-bar" aria-label="Score tonight">
      ...
      <Link href={week ? `/night?night=${week.id}` : "/night"} className="entry-tile"><Mic size={18}/> Say a roll</Link>
    </div>
