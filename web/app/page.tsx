...
export default function Home() {
  ...
  return <main>
    <header className="topbar"><a className="brand" href="/" aria-label="BA4L home"><span className="brand-icon"><CircleDot size={23}/></span>BA4L</a><div className="topbar-right"><a className="league-tag" href="/season"><span/> SEASON</a><a className="league-tag" href="/league"><span/> STANDINGS</a><AccountBar me={me} nightId={id} role={role} onClaimed={()=>void reload()}/></div></header>
    <section className="intro"><div><div className="eyebrow">EVERY PIN. EVERY POSSIBILITY.</div><h1>There’s still a <em>chance.</em></h1><p>Your score so far. Your best possible finish. All night long.</p></div>{shared&&id?<a className="secondary history-button" href={`/season/${id}`}><History size={17}/> This week</a>:<button className="secondary history-button" onClick={()=>setModal("history")}><History size={17}/> Game history</button>}</section>
    ...
    <section className="team-grid" aria-label="Team scoreboard">
      {names.map((name,i)=><button key={name} className={`bowler-card ${selected === i ? "selected" : ""}`} onClick={()=>setSelected(i)} aria-pressed={selected === i}>
    ...
    <div className="workspace">
      ...
      <aside className="side-panel"><MatchPanel night={night} setNight={setNight} disabled={!ready}/><TargetsPanel night={night}/><section className="team-summary"><div className="eyebrow"><Trophy size={16}/> THE BIG PICTURE</div><h3>Team potential</h3><strong className="team-total">{teamMax.toLocaleString()}</strong><p>Combined best possible score<br/>for this game.</p><div className="potential-track"><span style={{width:`${teamMax/12}%`}}/></div><div className="range-labels"><span>0</span><span>1,200 PERFECT GAME</span></div><div className="summary-divider"/><div className="summary-row"><span>Team score so far</span><strong>{states.reduce((a,s)=>a+s.score,0)}</strong></div><div className="summary-row"><span>Games recorded</span><strong>{night.history.length}</strong></div></section>
