...
export default function MatchPanel({ night, setNight, disabled }: { night: Night; setNight: (u: (n: Night) => Night) => void; disabled: boolean }) {
  ...
  return <section className="match-panel">
    ...
    {match && points ? <>
      ...
      <div className="opponent-grid-scroll" tabIndex={0} role="region" aria-label="Opponent scores, scroll for more columns"><table className="opponent-grid"><thead><tr><th scope="col" className="left">{title(match.opponent.name)}</th>{Array.from({ length: GAMES_PER_NIGHT }, (_, g) => <th scope="col" key={g}>G{g + 1}</th>)}<th scope="col">HDCP</th></tr></thead>
