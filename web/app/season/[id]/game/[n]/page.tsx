  ...
  return <main>
    ...
    {night && entry && summary && <>
      <section className="game-hero">
        ...
        <div className="game-hero-side">{summary.team != null ? <>Team {summary.team} scratch{gp && gp.ours != null && gp.theirs != null && <><br/><strong>{gp.ours} v {gp.theirs} hdcp</strong></>}</> : "In progress"}</div>
      </section>
      <div className="game-nav">
        {game > 1 ? <Link className="chip" href={`/season/${id}/game/${game - 1}`}>‹ Game {game - 1}</Link> : <span className="chip disabled">‹</span>}
        <span className="chip active">Game {game}</span>
        {game < last ? <Link className="chip" href={`/season/${id}/game/${game + 1}`}>Game {game + 1} ›</Link> : <span className="chip disabled">›</span>}
      </div>
      {BOWLERS.map((b, i) => { const st = analyze(entry.rolls[i]); const final = entry.finals?.[i]; const pb = points?.bowlers[i]; const split = pb?.games[game - 1]; const opp = night.match?.opponent.bowlers[i]; const oppScore = night.match?.opponentGames[game - 1]?.[i];
        return <section className="card game-section" key={b}>
          ...
          {entry.rolls[i].length ? <div className="scorecard-scroll"><table className="mini-card"><tbody><tr>{st.frames.map((f, k) => <td key={k}><div className="frame-rolls">{Array.from({ length: k === 9 ? 3 : 2 }, (_, j) => <span key={j}>{symbol(f.rolls, j) || "·"}</span>)}</div><strong>{f.score ?? ""}</strong></td>)}</tr></tbody></table></div>
            : <p className="score-note">{final != null ? "Final total only; frames were not recorded." : "Not bowled."}</p>}
          {entry.rolls[i].length > 0 && <p className="score-note">{st.frames.filter(f => f.rolls.length >= 2 && f.rolls[0] + f.rolls[1] < 10).length} open frames · {st.frames.filter(f => f.rolls[0] === 10).length} strikes</p>}
        </section>; })}
    </>}
